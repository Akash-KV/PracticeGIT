...
//    }
    ...
    public static void waitUntilAjaxLoaded(WebDriver driver) {
        while (true) {
            try {
                Boolean ajaxIsComplete = (Boolean) ((JavascriptExecutor) driver).executeScript("return window.jQuery != undefined && jQuery.active == 0");
                if (ajaxIsComplete) {
                    break;
                }
                Thread.sleep(1000);
